### SGHMC algorithm applied to the logistic regression model

### Created on 06/08/20 14:29 by A.Gozacan
...
grad_U <- function(beta, b_0, B_0, batch, predictors) {
...
runSim <- function( m, n, M, big_T, t_i, y_i, C, B_hat, epsilon_t, start_pos ) { # m = Interval size for "leapfrog" method, n = batch size
  ...
  for(t in 1:(big_T-1)) {
    ...
    for( i in 1:m ) {
      ...
      grad_U_tilde <- grad_U( beta_i[,i+1], b_0, B_0, y_batch, t_batch )
      
      r_i[,i+1] <- r_i[,i] - epsilon_t[t] * grad_U_tilde - epsilon_t[t] * ( C %*% solve(M) %*% r_i[,i] ) + noise[i,]
      
    }
      
    beta_t[,t+1] <- beta_i[,m+1]
  ...
  plot(1:big_T, beta_t[1, 1:big_T], type="l", xlab=paste(big_T, "Iterations"), ylab="Sampler of the posterior for beta_1")
  
  abline(h=true_beta[1], col="red")
  
  plot(1:big_T, beta_t[2, 1:big_T], type="l", xlab=paste(big_T, "Iterations"), ylab="Sampler of the posterior for beta_2")
  
  abline(h=true_beta[2], col="red")
  
  image(kde2d(beta_t[1, 1:big_T], beta_t[2, 1:big_T], n=200), col=hcl.colors(100, "YlGn", rev=TRUE), xlab="beta_1", ylab="beta_2")
  
  mtext(paste("SGHMC:", round(run_time, 3), "seconds"), side = 3, line = -44, outer = TRUE)
...
runSim( interval_size, minibatch_size, M, big_T, t_i, y_i, C, B_hat, epsilon_t, start_pos )
