### SGLD algorithm applied to the logistic regression model

### Created on 06/08/20 14:58 by A.Gozacan
...
runSim <- function( n, big_T, t_i, y_i, epsilon_t, start_pos, N ) { # n = batch size
  ...
  for(t in 1:(big_T-1)) {
    ...
    beta_t[,t+1] <- beta_t[,t] + delta( beta_t[,t], b_0, B_0, y_batch, t_batch, N, n, epsilon_t, eta_t, t )
  ...
  mtext(paste("SGLD:", round(run_time, 3), "seconds"), side = 3, line = -44, outer = TRUE)
...
runSim( minibatch_size, big_T, t_i, y_i, epsilon_t, start_pos, N )
