### SGLD + MTM + VR algorithm applied to the logistic regression model

### Created on 03/08/20 17:35 by A.Gozacan
...
runSim <- function( m, n, big_T, t_i, y_i, epsilon_t, u_j, start_pos, N ) { # m = Interval size for "leapfrog" method, n = batch size
  ...
  for(t in 1:(big_T-1)) {
    ...
    for( i in 1:m ) {
      ...
      beta_i[,i+1] <- beta_i[,i] + delta( beta_i[,i], b_0, B_0, y_batch, t_batch, N, n, epsilon_t, g_tilde, beta_tilde, eta_t, t )
    ...
    for(l in 1:L[1]) {
      ...
      for( i in 1:m ) {
        ...
        beta_i[,i+1] <- beta_i[,i] + delta( beta_i[,i], b_0, B_0, y_batch, t_batch, N, L[2], epsilon_t, g_tilde, beta_tilde, eta_t, t )
  ...
  mtext(paste("SGLD + MTM + VR with Correction:", round(run_time, 3), "mins. Avg acc. prob. =", round(mean(accept.probs), 3)), side = 3, line = -44, outer = TRUE)
