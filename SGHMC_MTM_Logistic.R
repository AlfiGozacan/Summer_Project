### SGHMC + MTM algorithm applied to the logistic regression model

### Created on 30/07/20 13:27 by A.Gozacan
...
runSim <- function( m, n, M, big_T, t_i, y_i, C, B_hat, epsilon_t, u_j, start_pos, L ) { # m = Interval size for "leapfrog" method, n = batch size, L = length of sub-samples for variance calculations
  ...
  mtext(paste("SGHMC + MTM with Correction:", round(run_time, 3), "mins. Avg acc. prob. =", round(mean(accept.probs), 3)), side = 3, line = -44, outer = TRUE)
